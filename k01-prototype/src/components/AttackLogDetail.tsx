...
import { BlockOutlined, SafetyCertificateOutlined, StarOutlined, StarFilled } from '@ant-design/icons';
...
const AttackLogDetail: React.FC<AttackLogDetailProps> = ({
  ...
  useEffect(() => {
    ...
    if (savedIps) {
      ...
      setIsFavorite(parsedIps.some((ip: any) => ip.ip === data.attackIp));
    }
  }, [data.attackIp]);

  const handleToggleFavorite = () => {
    const savedIps = localStorage.getItem('favoriteIps');
    let favoriteIps = savedIps ? JSON.parse(savedIps) : [];

    if (isFavorite) {
      favoriteIps = favoriteIps.filter((ip: any) => ip.ip !== data.attackIp);
