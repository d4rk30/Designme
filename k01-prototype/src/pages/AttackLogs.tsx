...
import { Card, Table, Button, Space, Input, Form, Select, Row, Col, Cascader, Modal, Dropdown, Menu, message } from 'antd';
        ...
        savedFilters.map(filter => (
          <Menu.Item key={filter.id}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', width: '250px' }}>
              ...
              <Space>
                <Button type="link" size="small" onClick={() => applyFilter(filter)}>
                  应用
                </Button>
                <Button type="link" size="small" danger onClick={() => deleteFilter(filter.id)}>
                  删除
                </Button>
              </Space>
            </div>
